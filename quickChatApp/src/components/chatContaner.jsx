  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            {messagesDummyData.map((msg, i)=>(
              <div key={i} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
                {msg.image ? (
                    ...
                    <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-blue-900/50 text-white ${msg.senderId === '680f50e4f10f3cd28382ecf9' ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
                )}
                <div className='text-center text-xs'>
                    ...
                    <p className='text-gray-800'>{formateDate(msg.createdAt)}</p>
                </div>
              </div>
            ))}
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
            <div className='flex-1 flex items-center bg-gray-600 px-3 rounded-full'>
            ...
            <img src={assets.send_button} alt='' className='w-7 cursor-pointer blue-900/50 '/>
        </div>
    </div>
  ) : (
